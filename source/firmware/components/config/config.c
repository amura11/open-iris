...
{
    iris_config_t *config = calloc(1, sizeof(iris_config_t));
    ...
    esp_err_t error = validate_header(reader, &config->root_state_id);
    ...
    error = load_section_indexes(reader, section_count, manifest_sections, config,
    ...
    error = populate_device_pool(reader, device_count, &device_section_index, config);
    free_section_index(&device_section_index);
    ...
    error = populate_function_pool(reader, function_count, config);
    ...
    error = populate_sequence_pool(reader, sequence_count, config);
    ...
    error = populate_state_pool(reader, state_count, config);
    ...
    ESP_LOGI(TAG, "loaded: %u devices, %u functions, %u sequences, %u states",
             config->device_count, config->function_count, config->sequence_count, config->state_count);
...
esp_err_t iris_config_load_buffer(const uint8_t *buffer, size_t buffer_length, iris_config_t **out_config)
{
    ...
    esp_err_t error = iris_config_load(reader, out_config);

    // Clear reader after eager load — all records are in pools, reader not needed.
    if (error == ESP_OK) {
        (*out_config)->reader = (iris_reader_t){0};
    }
...
{
    return iris_config_get_state(config, config->root_state_id);
}
