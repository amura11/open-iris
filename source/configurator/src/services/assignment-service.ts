...
import { stepToWireAction, configStore } from '@stores/config-store.svelte.ts';
...
export function assignPhysicalButtonSingleAction(button: ButtonDescriptor, step: SequenceStep): void {
    ...
    configStore.updateState({ ...activeState, physicalButtons: updatedPhysicalButtons });

    if (previousAssignment?.kind === 'sequence') {
        configStore.deleteAnonymousSequence(previousAssignment.sequenceId);
    }
}
...
export function assignPhysicalButtonSequence(button: ButtonDescriptor, steps: SequenceStep[], name: string | undefined, delayMs: number): void {
    ...
    if (previousAssignment?.kind === 'sequence') {
        configStore.updateSequence(previousAssignment.sequenceId, steps, name, delayMs);
        sequenceId = previousAssignment.sequenceId;
    } else {
        sequenceId = configStore.addSequence(steps, name, delayMs);
    }
...
export function assignScreenButtonSingleAction(screenButton: ScreenButton, step: SequenceStep): void {
    ...
    const updatedScreenButtons = activeState.screenButtons.map(b =>
        b.id === screenButton.id ? { ...b, assignment: stepToAssignment(step) } : b
    );
    configStore.updateState({ ...activeState, screenButtons: updatedScreenButtons });
...
export function assignScreenButtonSequence(screenButton: ScreenButton, result: SequenceEditorConfirmation): void {
    ...
    if (previousAssignment?.kind === 'sequence') {
        configStore.updateSequence(previousAssignment.sequenceId, result.steps, result.name, result.delayMs);
        sequenceId = previousAssignment.sequenceId;
    } else {
        sequenceId = configStore.addSequence(result.steps, result.name, result.delayMs);
    }

    const updatedScreenButtons = activeState.screenButtons.map(b =>
        b.id === screenButton.id ? { ...b, assignment: { kind: 'sequence' as const, sequenceId } } : b
    );
    configStore.updateState({ ...activeState, screenButtons: updatedScreenButtons });
}
...
export function removeScreenButtonAssignment(screenButton: ScreenButton): void {
    ...
    const updatedScreenButtons = activeState.screenButtons.map(b =>
        b.id === screenButton.id ? { ...b, assignment: null } : b
    );
    configStore.updateState({ ...activeState, screenButtons: updatedScreenButtons });
